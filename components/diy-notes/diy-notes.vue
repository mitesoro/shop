<template>
	<view class="diy-notes" v-if="dataList.length" :style="{ backgroundColor: value.backgroundColor, marginTop: value.marginTop * 2 + 'rpx' }">
		<view class="diy-notes-top">
			<view class="notes-title">
				<block v-if="value.list">
					<text class="notes-title-name" :style="{ color: value.titleTextColor ? value.titleTextColor : 'rgba(0,0,0,0)' }">{{ value.list.title }}</text>
				</block>
			</view>
			<block v-if="value.listMore">
				<view class="notes-more" @click="toMore()" :style="{ color: value.moreTextColor ? value.moreTextColor : 'rgba(0,0,0,0)' }">{{ value.listMore.title }}</view>
			</block>
		</view>

		<view class="diy-notes-boxs">
				...
				<view class="notes-box-item" v-for="(item, i) in dataList" :key="i" @click="toDetail(item.note_id)">
					<view class="notes-item">
						...
						<view class="notes-img-wrap" :class="{ 'notes-img-wrap-list': item.cover_type == 1 }">
							<image v-if="item.cover_type == 0" :src="$util.img(item.img)" @error="imgError(i)" mode="aspectFill" class="notes-item-image"></image>
							<image
								v-else
...
export default {
	...
	methods: {
		...
		getDataList() {
			var data = { page: 1, page_size: this.value.goodsCount };
		...
		imgError(index) {
			this.dataList[index].img = this.$util.getDefaultImage().default_goods_img;
			this.$forceUpdate();
		}
	}
};
