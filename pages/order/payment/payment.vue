	...
	<view class="order-container" :class="{ 'safe-area': isIphoneX }" :data-theme="themeStyle">
		...
		<view class="mobile-wrap" v-if="orderPaymentData.is_virtual == 1 && orderCreateData.member_address">
			...
			<view class="form-group">
				...
				<input
					...
					class="input"
					v-model="orderPaymentData.member_account.mobile"
				/>
			</view>
		</view>
		...
		<view class="site-wrap" v-for="(siteItem, siteIndex) in orderPaymentData.shop_goods_list" :key="siteIndex">
			...
			<view class="site-body">
				...
				<view class="goods-wrap" v-for="(goodsItem, goodsIndex) in siteItem.goods_list" :key="goodsIndex">
					...
					<view class="goods-info">
						...
						<text class="tip color-base-bg" v-if="goodsItem.is_present == 1">赠品</text>
					</view>
				</view>
				<!-- 赠送优惠券 -->
				<block v-if="siteItem.present_list.coupon_list">
					<view class="order-cell" v-for="(coupon_item, coupon_index) in siteItem.present_list.coupon_list" :key="coupon_index">
			...
			<view class="site-footer">
				...
				<view class="order-cell coupon" v-if="siteItem.coupon_list.length">
					...
					<view class="box text-overflow shop-item" @click="openSiteCoupon(siteItem.site_id, siteItem.coupon_list)">
		...
		<uni-popup ref="couponPopup" type="bottom">
			<view class="coupon-popup popup" @touchmove.prevent.stop>
				...
				<view class="popup-footer" :class="{ 'bottom-safe-area': isIphoneX }">
					<view class="confirm-btn color-base-bg" @click="popupConfirm('couponPopup', siteCoupon.site_id)">确定</view>
				</view>
			</view>
		</uni-popup>
					...
					<view>
						<view class="order-cell" v-for="(promotionItem, promotionIndex) in sitePromotion" :key="promotionIndex">
							<template v-if="promotionIndex == 'manjian'">
								<text class="tit"><text class="promotion-mark ns-gradient-promotionpages-payment">优惠</text></text>
								<view class="box">
									<view v-for="(item, index) in promotionItem" :key="index" v-if="item.discount_array">
									{{item.discount_array.rule.free_shipping?'包邮':'' }}
									{{item.discount_array.rule.coupon>0?'满'+item.discount_array.rule.money+'元送优惠券':'' }}
									{{item.discount_array.rule.discount_money>0?'满'+item.discount_array.rule.money+'元减'+item.discount_array.rule.discount_money+'元':'' }}
									{{item.discount_array.rule.present?'送赠品':'' }}
									</view>
								</view>
							</template>
