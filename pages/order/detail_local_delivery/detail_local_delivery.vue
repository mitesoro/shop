	...
	<view class="detail-container" :class="{ 'safe-area': isIphoneX }" :data-theme="themeStyle">
		...
		<view class="address-wrap">
			...
			<view v-if="orderData.buyer_ask_delivery_time" class="pick-block first-pick-block">
				...
				<view class="last-child">{{ orderData.buyer_ask_delivery_time | timeStr }}</view>
			</view>
		...
		onShow() {
			...
			if (uni.getStorageSync('token')) {
				...
				this.$util.redirectTo('/pages/login/login/login', {
					back: '/pages/order/detail_local_delivery/detail_local_delivery?order_id=' + this.orderId
				});
			}
			this.getKekuConfig();
		},
		methods: {
			...
			goConnect() {
				if (uni.getStorageSync('token')) {
					...
					this.$refs.login.open('/pages/goods/detail_local_delivery/detail_local_delivery?sku_id=' + this.orderData.sku_id);
					return;
				}
			},
			getKekuConfig() {
