								...
								<view class="goods-info">
									...
									<view class="goods-sub-section">
										<text class="goods-price">
											...
											<text class="color-base-text">{{ orderItem.price }}</text>
										</text>
							...
							<view class="order-action" v-if="orderItem.action.length > 0">
								<block v-for="(operationItem, operationIndex) in orderItem.action" :key="operationIndex">
									<block v-if="operationItem.action == 'orderPayFinal'">
										<block v-if="orderItem.refund_status == 0 || orderItem.refund_status == -1">
											<view class="order-box-btn color-base-border color-base-text" @click="operation(operationItem.action, orderItem,isBalance)" v-if="orderItem.pay_start_time < timestamp && orderItem.pay_end_time > timestamp">{{ operationItem.title }}</view>
											<view class="order-box-btn disabled" v-else>{{ operationItem.title }}</view>
										</block>
										<view class="order-box-btn disabled" v-else>{{ operationItem.title }}</view>
									</block>
									<block v-else-if="operationItem.action == 'refundDeposit' && orderItem.is_deposit_back == 0">
										<view class="order-box-btn" v-if="orderItem.refund_status == 0" @click="operation(operationItem.action, orderItem,isBalance)">{{ operationItem.title }}</view>
									</block>
									<view class="order-box-btn" @click="operation(operationItem.action, orderItem,isBalance)" v-else>{{ operationItem.title }}</view>
								</block>
							</view>
...
export default {
	...
	onShow() {
		...
		if (uni.getStorageSync('token')) {
			...
			this.$util.redirectTo('/pages/login/login/login', {
				back: '/promotionpages/presale/order_list/order_list'
			});
		}
	...
	methods: {
		...
		operation(action,orderItem) {
			switch (action) {
				case 'deleteOrder': // 删除
					this.deleteOrder(orderItem.id,() => {
						this.$refs.mescroll.refresh();
					});
					break;
				case 'orderClose': //关闭
					this.orderClose(orderItem.id, () => {
						this.$refs.mescroll.refresh();
					});
					break;
				case 'orderPayDeposit'://定金支付
					this.orderData = orderItem
					this.openPaymentPopup(orderItem, 'presale_deposit_money');
					break;
				case 'refundDeposit'://退定金
					this.refundDeposit(orderItem.id, () => {
						this.$refs.mescroll.refresh();
					});
					break;
				case 'orderPayFinal': //支付尾款
					this.orderData = orderItem
					this.openPaymentPopup(orderItem, 'final_money');
					break;
			}
		},
		orderDetail(orderData) {
			this.$util.redirectTo('/promotionpages/presale/order_detail/order_detail',{order_id: orderData.id})
		},
