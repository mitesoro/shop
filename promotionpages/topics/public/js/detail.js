...
export default {
	...
	methods: {
		...
		async getGoodsSkuDetail() {
			...
			if (data.goods_sku_detail != null) {
				...
				this.contactData = {
					...
					path: '/promotionpages/topics/detail/detail?id=' + this.topics_id,
					...
					success: res => {
						if (res.code == 0) {
							...
							let shareConfig = res.data.share_config,
								url = this.$config.h5Domain + '/promotionpages/topics/goods_detail/goods_detail?id=' + this.topics_id;
							if (this.memberId) url += '&suid=' + this.memberId;

							wxJS.setShareData({
								title: this.goodsSkuDetail.sku_name,
								desc: shareConfig.goods_param_1 + '：' + this.$lang('common.currencySymbol') + this.goodsSkuDetail.topic_price +
		...
		copyUrl() {
			let text = this.$config.h5Domain + '/promotionpages/topics/goods_detail/goods_detail?id=' + this.topics_id;
