<template>
	<view class="bargain" :style="{ backgroundImage: 'url(' + $util.img('upload/uniapp/bargain/bargain_background1.png') + ')' }" :data-theme="themeStyle">
		<view class="bargain_top" :style="{ backgroundImage: 'url(' + $util.img('upload/uniapp/bargain/bargain_background.png') + ')' }"></view>
		<view class="bargin_content">
			<view class="uer_info_base">
				<view class="user_info_img">
					<image :src="info.headimg == '' ? $util.img($util.getDefaultImage().default_headimg) : $util.img(info.headimg)" mode="widthFix"></image>
				</view>
			...
			<!-- 商品 -->
			<view class="goods">
				<view class="goods_img"><image :src="$util.img(info.sku_image)" mode="aspectFit"></image></view>
				<view class="goods_content">
					<view class="goods_title">{{ info.sku_name }}</view>
					<view class="goods_price">
						<view>
							原价
							<text class="original_price">{{ $lang('common.currencySymbol') }}{{ info.price }}</text>
						</view>
						<view>
							最低可砍至
							<text class="bottom_price">{{ $lang('common.currencySymbol') }}{{ info.floor_price }}</text>
						</view>
					</view>
					<view class="partake_num">
						已有
						<text class="ident">{{ info.curr_num }}</text>
						人帮砍
						<text class="residue-num">
							剩余数量
							<text class="ident">{{ info.bargain_stock }}</text>
						</text>
					</view>
					<view v-if="timeMachine">
						还剩
						<uni-count-down
							:day="timeMachine.d"
							:hour="timeMachine.h"
			...
			<view class="progress_info">
				<view>
					已砍至
					<text>{{ info.curr_price | moneyFormat }}</text>
					元
				</view>
				<view>最低{{ info.floor_price }}元</view>
			</view>
			<view class="progress">
				<view class="progress_item" :style="{ width: (progress > 5 ? progress : 5) + '%' }">
					<image class="progress_img" :src="$util.img('upload/uniapp/bargain/bargain_progress.png')"></image>
				</view>
			</view>
				...
				<view class="bragain_recode">
					<view class="bragain_recode_list">
						<block v-for="(item, index) in bargainRecord" :key="index">
							<image :src="item.headimg == '' ? $util.img($util.getDefaultImage().default_headimg) : $util.img(item.headimg)" v-if="index < 4"></image>
						</block>
						<block v-if="bargainRecord.length < 5"><view class="bragain_recode_add" v-for="(item, index) in 5 - bargainRecord.length">+</view></block>
					</view>
			...
			<view class="invitation_peo">
				<view v-if="info.self" class="flex-box">
					...
					<block v-if="info.status == 1">
						<button class="btn-vice" @click="$util.redirectTo('/promotionpages/bargain/list/list')">低价拿更多商品</button>
						<button class="btn" @click="buyNow" v-if="info.order_id == 0">立即购买</button>
					</block>
					<block v-if="info.status == 2"><button class="btn" @click="$util.redirectTo('/promotionpages/bargain/list/list')">低价拿更多商品</button></block>
				</view>
				<view v-else class="flex-box">
					<block v-if="info.status == 0">
						<button class="btn" @click="bargain" v-if="!info.cut">帮好友砍一刀</button>
						<button class="btn-vice" @click="$util.redirectTo('/promotionpages/bargain/list/list')" v-else>我也要低价拿</button>
					</block>
					<block v-else><button class="btn" @click="$util.redirectTo('/promotionpages/bargain/list/list')">我也要低价拿</button></block>
				</view>
			</view>
		</view>
		<view class="bargin_introduction">
			<view class="bargin_introduction_title">
				<image class="left" :src="$util.img('upload/uniapp/bargain/bargain_left.png')"></image>
				砍价流程
				<image class="right" :src="$util.img('upload/uniapp/bargain/bargain_right.png')"></image>
			</view>
			<view class="bargin_introduction_content">
				<view class="flow">
					<view class="flow_item">
						<image :src="$util.img('upload/uniapp/bargain/bargain_kanjia.png')"></image>
						<view>发起商品砍价</view>
					</view>
					<view class="flow_item">
						<image :src="$util.img('upload/uniapp/bargain/bargain_friend.png')"></image>
						<view>邀请好友砍价达要求</view>
					</view>
					<view class="flow_item">
						<image :src="$util.img('upload/uniapp/bargain/bargain_good.png')"></image>
						<view>领取商品带回家</view>
					</view>
				</view>
			</view>
				...
				<view class="bargin_invitation" v-if="bargainRecord.length">
					<view class="item" v-for="(item, index) in bargainRecord" :key="index">
						<view class="item_left">
							<image :src="item.headimg == '' ? $util.img($util.getDefaultImage().default_headimg) : $util.img(item.headimg)"></image>
							<view>
		...
		this.$store.dispatch('getAddonIsexit').then(data => {
			if (!data.bargain) {
				...
				if (uni.getStorageSync('token')) {
					...
					var back = encodeURIComponent('/promotionpages/bargain/launch/launch?id=' + this.id);
	...
	methods: {
		...
		bargain() {
			if (uni.getStorageSync('token')) {
				...
				this.$refs.login.open('/promotionpages/bargain/launch/launch?id=' + this.id);
			}
		},
		getMemberId() {
			this.$api.sendRequest({
		...
		copyUrl() {
			// #ifdef H5
			let text = '嘿！朋友就差你这一刀了，帮一下忙呗~' + this.$config.h5Domain + '/promotionpages/bargain/launch/launch?id=' + this.id;
			...
			uni.share({
				...
				type: 0,
				href: Config.h5Domain + '/promotionpages/bargain/launch/launch?id=' + this.id,
				title: this.info.sku_name,
				summary: this.info.sku_name,
