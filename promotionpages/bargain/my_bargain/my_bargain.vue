					...
					<view class="goods-list" v-for="(item, index) in dataList" :key="index">
						<view class="goods-item-content" @click="goBargainDetail(item.launch_id)">
							...
							<view class="goods-item-wrap">
								<view class="image-wrap"><image :src="$util.img(item.sku_image)" mode="aspectFit" @error="imgError(index)"></image></view>
								<view class="content">
						...
						<view class="goods-item-action">
							<view class="invitation-bargain">
								<image
									:src="recordItem.headimg == '' ? $util.img($util.getDefaultImage().default_headimg) : $util.img(recordItem.headimg)"
									v-for="(recordItem, recordIndex) in item.bargain_record"
									@error="memberImgError(index, recordIndex)"
								></image>
