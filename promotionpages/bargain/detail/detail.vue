	...
	<view :data-theme="themeStyle">
		<view scroll-y="true" class="goods-detail" :class="isIphoneX ? 'active' : ''">
			<view class="goods-container">
				...
				<view v-if="goodsSkuDetail.timeMachine" class="goods-promotion" :style="{ backgroundImage: 'url(' + $util.img('upload/uniapp/goods/detail_promotion_left_bg.png') + ')' }">
					<view class="price-info">
						<view class="img-wrap">
							<image :src="$util.img('upload/uniapp/goods/detail_bargain_tag.png')" mode="aspectFit"></image>
						</view>
					...
					<view class="countdown" :style="{ backgroundImage: 'url(' + $util.img('upload/uniapp/goods/detail_promotion_right_bg.png') + ')' }">
						...
						<view class="clockrun">
							<uni-count-down :day="goodsSkuDetail.timeMachine.d" :hour="goodsSkuDetail.timeMachine.h" :minute="goodsSkuDetail.timeMachine.i"
							 :second="goodsSkuDetail.timeMachine.s" color="#fff" splitorColor="#FF4644" backgroundColor="#FF4644" />
						</view>
					</view>
				</view>
				...
				<view class="group-wrap padding-top">
					<view class="goods-module-wrap">
						<text class="promotion-tag color-base-bg">砍成价</text>
				...
				<!-- 当前商品参与的营销活动入口 -->
				<!-- <view class="group-wrap"><ns-goods-promotion ref="goodsPromotion" promotion="bargain"></ns-goods-promotion></view> -->

				<view class="newdetail margin-bottom">
					<!-- 已选规格 -->
					<view class="item selected-sku-spec" v-if="goodsSkuDetail.sku_spec_format" @click="bargain">
				...
				<!-- SKU选择 -->
				<ns-goods-sku ref="goodsSku" @refresh="refreshGoodsSkuDetail" :goods-detail="goodsSkuDetail" :maxBuy="maxBuy"></ns-goods-sku>
		...
		<ns-goods-action :safeArea="isIphoneX">
			<template v-if="goodsSkuDetail.goods_state == 1 && goodsSkuDetail.verify_state == 1">
				...
				<block v-if="goodsSkuDetail.launch_info != undefined">
					<ns-goods-action-button class="goods-action-button active4" text="正在砍价" @click="$util.redirectTo('/promotionpages/bargain/launch/launch', { id: goodsSkuDetail.launch_info.launch_id })" />
				</block>
				<block v-else>
					<ns-goods-action-button class="goods-action-button active4" :text-price="'最低¥ ' + goodsSkuDetail.show_price" text="去砍价"
					 @click="bargain" v-if="goodsSkuDetail.bargain_stock > 0 || (goodsSkuDetail.bargain_stock == 0 && goodsSkuDetail.sku_spec_format)" />
					<block v-else-if="goodsSkuDetail.bargain_stock == 0 && !goodsSkuDetail.sku_spec_format">
						<ns-goods-action-button class="goods-action-button active3" disabled-text="库存不足" :disabled="true" />
						<!-- <ns-goods-action-button v-if="goodsSkuDetail.sku_spec_format" class="goods-action-button active3" disabled-text="库存不足" :disabled="true" @click="bargain" />
...
<script>
	...
	export default {
		...
		onShareAppMessage(res) {
			var path = '/promotionpages/bargain/detail/detail?bargain_id=' + this.bargain_id;
		...
		// 分享到微信朋友圈
		onShareTimeline() {
			let query = 'bargain_id=' + this.bargain_id;
