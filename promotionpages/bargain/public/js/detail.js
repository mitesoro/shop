					...
					success: res => {
						if (res.code == 0) {
							...
							let shareConfig = res.data.share_config,
								url = this.$config.h5Domain + '/promotionpages/bargain/detail/detail?bargain_id=' + this.bargain_id;
							if (this.memberId) url += '&suid=' + this.memberId;

							wxJS.setShareData({
								title: this.goodsSkuDetail.sku_name,
								desc: '底价' + '：' + this.$lang('common.currencySymbol') + this.goodsSkuDetail.show_price + '\r\n' +
