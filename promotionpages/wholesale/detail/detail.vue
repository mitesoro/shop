	...
	<view :data-theme="themeStyle">
			...
			<view class="goods-container">
				...
				<view class="group-wrap padding-top">
					<view class="goods-module-wrap">
						<text class="promotion-tag color-base-bg">批发价</text>
						<text class="price-symbol color-base-text">{{ $lang('common.currencySymbol') }}</text>
						<text class="price color-base-text">{{ goodsSkuDetail.min_price }}</text>
				...
				<view class="newdetail margin-bottom">
					<view class="item" v-for="(item, index) in goodsSkuDetail.price_array" :key="index">
						<text class="price-symbol color-base-text">{{ $lang('common.currencySymbol') }}</text>
						<text class="price color-base-text margin-right">{{ item.price }}</text>
						<text class="startNumber">{{ item.num }}{{ goodsSkuDetail.unit }}起批</text>
					</view>
				</view>

				<view class="newdetail margin-bottom">
					<!-- 已选规格 -->
					<view class="item selected-sku-spec" v-if="goodsSkuDetail.sku_spec_format" @click="wholesaleJoinCart">
		...
		<ns-goods-action :safeArea="isIphoneX">
			<template v-if="goodsSkuDetail.goods_state == 1 && goodsSkuDetail.verify_state == 1">
				...
				<ns-goods-action-icon text="进货单" icon="icongouwuche21" :corner-mark="wholeSaleNumber > 0 ? wholeSaleNumber + '' : ''"
				 @click="goCart" />
				<block v-if="goodsSkuDetail.stock == 0 && !goodsSkuDetail.sku_spec_format">
					...
					<!-- <ns-goods-action-button v-if="goodsSkuDetail.sku_spec_format" class="goods-action-button active3" disabled-text="库存不足" :disabled="true" @click="wholesaleJoinCart" /> -->
					<!-- <ns-goods-action-button v-else class="goods-action-button active3" disabled-text="库存不足" :disabled="true" /> -->
				</block>
				<block v-else>
					<ns-goods-action-button class="goods-action-button" :class="goodsSkuDetail.is_virtual == 0 ? 'active1' : ''" text="加入进货单"
					 background="color-join-cart" @click="wholesaleJoinCart" v-if="goodsSkuDetail.is_virtual == 0" />
					<ns-goods-action-button class="goods-action-button" :class="goodsSkuDetail.is_virtual == 0 ? 'active2' : 'active4'"
					 text="立即购买" @click="wholesaleBuyNow" />
				</block>
			</template>
			<template v-else>
...
<script>
	...
	export default {
		...
		// 分享到微信朋友圈
		onShareTimeline() {
			let query = 'goods_id=' + this.goodsId;
