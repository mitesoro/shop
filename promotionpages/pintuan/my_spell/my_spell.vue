			...
			<block slot="list">
				<view class="goods-list" v-for="(item, index) in dataList" :key="index">
					...
					<view class="list-body" @click="goPinTuanDetail(item.pintuan_id)">
						<view class="list-body-img"><image :src="$util.img(item.sku_image, { size: 'mid' })" @error="imgError(index)"></image></view>
					...
					<view v-else-if="item.pintuan_status == 3" class="list-footer">
						<view class="picture-box">
							<view class="img-box" v-for="(i, j) in item.member_list" v-if="j < 4" :key="j">
								<image v-if="i.member_img" :src="$util.img(i.member_img)" @error="memberImgError(index, j)" mode="aspectFill"></image>
				...
				<view v-if="dataList.length == 0" style="padding-top:0"><ns-empty text="暂无拼团订单" :emptyBtn="emptyBtn"></ns-empty></view>
			</block>
		</mescroll-uni>
