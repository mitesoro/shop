	...
	<view :data-theme="themeStyle">
			...
			<view class="goods-container">
				...
				>
					<view class="price-info">
						<view class="img-wrap"><image :src="$util.img('upload/uniapp/goods/detail_groupbuy_tag.png')" mode="aspectFit"></image></view>
				...
				<view class="group-wrap padding-top">
					<view class="goods-module-wrap">
						<text class="promotion-tag color-base-bg">团购价</text>
				...
				<!-- 当前商品参与的营销活动入口 -->
				<!-- <view class="group-wrap"><ns-goods-promotion ref="goodsPromotion" promotion="groupbuy"></ns-goods-promotion></view> -->
		...
		<ns-goods-action :safeArea="isIphoneX">
			<template v-if="goodsSkuDetail.goods_state == 1 && goodsSkuDetail.verify_state == 1">
				...
				<ns-goods-action-button v-else class="goods-action-button active1" text="立即抢购" @click="groupbuy" />
			</template>
			<template v-else>
...
export default {
	...
	onShareAppMessage(res) {
		var path = '/promotionpages/groupbuy/detail/detail?groupbuy_id=' + this.groupbuy_id;
	...
	// 分享到微信朋友圈
	onShareTimeline() {
		let query = 'groupbuy_id=' + this.groupbuy_id;
