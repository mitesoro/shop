	...
	<view class="container" :data-theme="themeStyle">
		<view class="content">
			...
			<view class="get-coupon">
				...
				<view class="color-base-text coupon-price">￥{{ info.money }}</view>
				...
				<view v-else class="color-tip">领取之日起{{ info.fixed_term }}天内有效</view>
				...
				if (data.scene) {
					...
					if (sceneParams.length) {
						sceneParams.forEach(item => {
							...
							if (item.indexOf('suid') != -1) this.suid = item.split('-')[1];
						});
					}
				}
			}
