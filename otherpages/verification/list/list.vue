<template>
	<view class="verify-container" :data-theme="themeStyle">
		...
		<swiper :current="type" class="swiper-box" style="flex: 1;" :duration="200" @change="ontabchange">
			<swiper-item class="swiper-item" :key="typeIndex" v-for="(typeItem, typeIndex) in typeList">
							...
							<view @click="toDetail(item.verify_code)">
								...
								<view class="body">
									<view class="content-item" v-for="(citem, citemIndex) in item.item_array" :key="citemIndex">
										<view class="img-wrap">
											<image :src="$util.img(citem.img)" @error="imgError(typeIndex, index, citemIndex)" mode="aspectFill"></image>
										</view>
				...
				<block v-else>
					<ns-empty :isIndex="!1" text="暂无核销记录!"></ns-empty>
				</block>
			</swiper-item>
		</swiper>
...
<script>
	...
	export default {
		...
		methods: {
			...
			imgError(typeIndex, index, citemIndex) {
