<template>
	<view>
		<view class="site-msg-order">
			<mescroll-uni ref="mescroll" @getData="getSiteMemberSubMessageDeliveryList">
				<block slot="list">
					<view class="flex-row-center" v-for="(item, index) in deliveryList" :key="index" v-if="deliveryList.length > 0">
									...
									<view class="order-item">
										...
										<view class="order-detail">
											...
											<view class="order-content">
												...
												<view class="view-order color-base-border color-base-text" type="default">查看物流</view>
											</view>
										</view>
									</view>
								</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			// 获取订单消息
			getSiteMemberSubMessageDeliveryList(mescroll) {
				this.$api.sendRequest({
					url: '/sitemessage/api/sitemessage/getSiteMemberSubMessageDeliveryList',
					data: {
						page: mescroll.num,
					...
					success: res => {
						...
						//设置列表数据
						if (mescroll.num == 1) this.deliveryList = []; //如果是第一页需手动制空列表
						this.deliveryList = this.deliveryList.concat(newArr); //追加新数据
