				...
				<view class="withdraw-li" v-for="(item, index) in withdrawList" :key="index">
					<view class="li-box">
						...
						<view class="money-desc">
							...
							<text class="color-base-text">￥{{ item.money }}</text>
						</view>
					</view>
				</view>
				<block v-if="withdrawList.length == 0 && emptyShow">
					<ns-empty :text="'暂无' + fenxiaoWords.withdraw + '记录'" :isIndex="!1" v-if="status == 0"></ns-empty>
					<ns-empty :text="'暂无待审核' + fenxiaoWords.withdraw + '记录'" :isIndex="!1" v-if="status == 1"></ns-empty>
					<ns-empty :text="'暂无已审核' + fenxiaoWords.withdraw + '记录'" :isIndex="!1" v-if="status == 2"></ns-empty>
					<ns-empty :text="'暂无已拒绝' + fenxiaoWords.withdraw + '记录'" :isIndex="!1" v-if="status == -1"></ns-empty>
				</block>
			</block>
		</mescroll-uni>
