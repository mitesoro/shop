			...
			<view class="level-top color-base-bg">
				<view class="head-img-wrap">
					<view>
						<view class="head-img">
							<image
								:src="fenxiaoInfo.headimg ? $util.img(fenxiaoInfo.headimg) : $util.getDefaultImage().default_headimg"
								@error="fenxiaoInfo.headimg = $util.getDefaultImage().default_headimg"
								mode="aspectFill"
							></image>
						</view>
						<view class="head-bg color-base-text" :style="{ backgroundImage: 'url(' + $util.img('upload/uniapp/fenxiao/level/bg2.png') + ')' }">
						...
						<block v-for="(item, index) in levelList" :key="index">
							<view class="level_list">
								<view class="level_list-top">
									<image :src="$util.img('upload/uniapp/fenxiao/level/v1.png')" />
									<text>{{ index + 1 }}</text>
								</view>
				...
				<view class="level-top-content">
					...
					<view class="level-img-item">
						<view class="level-img" v-if="Number(config.level) > 0">
							<view class="level-img-wrap"><image :src="$util.img('upload/uniapp/fenxiao/level/money.png')"></image></view>
							<view class="level-img2-wrap"><view class="content-name">一级佣金</view></view>
							<view class="level-fs">{{ levelInfo.one_rate }}%</view>
						</view>
						<view class="level-img" v-if="Number(config.level) > 1">
							<view class="level-img-wrap"><image :src="$util.img('upload/uniapp/fenxiao/level/money.png')"></image></view>
							<view class="level-img2-wrap"><view class="content-name">二级佣金</view></view>
							<view class="level-fs">{{ levelInfo.two_rate }}%</view>
						</view>
						<view class="level-img" v-if="Number(config.level) > 2">
							<view class="level-img-wrap"><image :src="$util.img('upload/uniapp/fenxiao/level/money.png')"></image></view>
							<view class="level-img2-wrap"><view class="content-name">三级佣金</view></view>
			...
			<view class="level-bottom" v-if="fenxiaoInfo.condition.last_level">
				<view class="level-bottom-name">
					...
					<view class="level_img">
						<image :src="$util.img('upload/uniapp/fenxiao/level/v_h.png')"></image>
						<text>{{ levelInfo.level_num }}</text>
					</view>
				</view>
				...
				<view class="detail">
					<view class="detail-wrap">
						...
						<view class="detail-item" v-if="fenxiaoInfo.condition.last_level.child_num > 0">
							<view class="detail-item-name">一级下线人数</view>
							<view class="detail-item-content">
								<text class="color-base-text">{{ fenxiaoInfo.child_num ? fenxiaoInfo.child_num : 0 }}</text>
								/ {{ fenxiaoInfo.condition.last_level.child_num }}
							</view>
						</view>
						<view class="detail-item" v-if="fenxiaoInfo.condition.last_level.child_fenxiao_num > 0">
							<view class="detail-item-name">一级下线分销商</view>
							<view class="detail-item-content">
								<text class="color-base-text">{{ fenxiaoInfo.child_fenxiao_num ? fenxiaoInfo.child_fenxiao_num : 0 }}</text>
								/ {{ fenxiaoInfo.condition.last_level.child_fenxiao_num }}
							</view>
						</view>
					</view>
				</view>
...
export default {
	...
	onShow() {
		...
		if (!this.addonIsExit.fenxiao) {
			...
			this.$langConfig.title(this.fenxiaoWords.fenxiao_name + '等级');
				...
				success: res => {
					if (res.code >= 0 && res.data) {
						...
						if (this.fenxiaoInfo.condition && this.fenxiaoInfo.condition.last_level) {
							this.fenxiaoInfo.condition.last_level.one_fenxiao_order_money = this.fenxiaoInfo.condition.last_level.one_fenxiao_order_money
								? Number(this.fenxiaoInfo.condition.last_level.one_fenxiao_order_money)
								: 0;
							this.fenxiaoInfo.condition.last_level.order_money = this.fenxiaoInfo.condition.last_level.order_money
								? Number(this.fenxiaoInfo.condition.last_level.order_money)
								: 0;
						}
						this.getLevelInfo();
					} else {
