<template>
	<view class="chat-message" :data-theme="themeStyle">
		<block v-if="message.contentType == 'sendGood'">
			<ns-chat-goods :isCanSend="send" :skuId="message.sku_id" @sendMsg="sendGood"></ns-chat-goods>
		</block>
		<block v-if="message.contentType == 'sendOrder'">
			<ns-chat-order :isCanSend="send" :orderId="message.order_id" @sendMsg="sendOrder"></ns-chat-order>
		</block>
		...
		<view class="message" v-if="message.contentType == 'string'">
			<view class="message-item " :class="message.isItMe ? 'right' : 'left'">
				...
				<block v-else>
					<view class="head_img">
						<image class="img" :src="$util.img(message.avatar)" mode="aspectFit" v-if="message.avatar"></image>
						<image class="img" :src="defaultAvatar" mode="aspectFit" v-else></image>
					</view>
				</block>
				<view class="chat_text">
					...
					<view class="content">
						<rich-text :nodes="stringToEmjoy(message.content)"></rich-text>
					</view>
		...
		<view v-else-if="message.contentType == 'goods'">
			<ns-chat-goods :isCanSend="!1" :skuId="message.sku_id"></ns-chat-goods>
		</view>
		<view v-else-if="message.contentType == 'order'">
			<ns-chat-order :isCanSend="!1" :orderId="message.order_id"></ns-chat-order>
		</view>
...
<script>
	...
	import nsChatGoods from '@/otherpages/components/ns-chat/ns-chat-goods.vue';
	import nsChatOrder from '@/otherpages/components/ns-chat/ns-chat-order.vue';
	import nsChatReceiveGoods from '@/otherpages/components/ns-chat/ns-chat-receiveGoods.vue'
	...
	export default {
		...
		data() {
			return {
				avatar: '', //店铺头像
				defaultAvatar: this.$util.getDefaultImage().default_shop_img,
				myHeadImg: '', //我的头像
