<template>
	<view class="signin" :data-theme="themeStyle">
		<!-- #ifdef H5 -->
		<view class="head-nav color-base-bg"></view>
		<!-- #endif -->
		<view class="sigin-box">
			<view class="sigin-bg"><image :src="$util.img('/upload/uniapp/signin/bg4.png')" mode="widthFix"></image></view>
			...
			<view class="signin-wrap">
				<view class="member-info">
					<view class="headimg">
						<view class="headimg-img">
							<image
								:src="headimg ? $util.img(headimg) : $util.getDefaultImage().default_headimg"
								mode="aspectFill"
								@error="headimg = $util.getDefaultImage().default_headimg"
							></image>
						</view>
					...
					<view class="point-box">
						<text class="point font-size-base">积分：{{ signPoint }}</text>
					</view>
				</view>
							...
							<scroll-view class="signin-day-scroll" scroll-with-animation="true" :scroll-x="true" :show-scrollbar="false" :scroll-into-view="'id_' + showDay">
								<block v-for="(item, index) in showSignDays">
									<view :id="'id_' + item.day" class="signin-day-item" v-if="item.day < signDaysSeries || (item.day == signDaysSeries && hasSign == 0)">
										...
										<image :src="$util.img('/upload/uniapp/signin/signed.png')"></image>
									</view>
									<view :id="'id_' + item.day" class="signin-day-item" v-else-if="item.day == signDaysSeries && hasSign == 1" style="background: #FF4544;">
										...
										<image :src="$util.img('/upload/uniapp/signin/sginin.png')"></image>
									</view>
									<view :id="'id_' + item.day" class="signin-day-item" v-else-if="$util.inArray(item.day, rewardRuleDay) != -1">
										<view class="">第{{ item.day }}天</view>
										<image :src="$util.img('/upload/uniapp/signin/award.png')"></image>
									</view>
									<view :id="'id_' + item.day" class="signin-day-item" v-else>
										<view class="">第{{ item.day }}天</view>
										<image :src="$util.img('/upload/uniapp/signin/gold1.png')"></image>
									</view>
								</block>
							</scroll-view>
						</view>
				...
				<view class="my-signin">
					...
					<view class="my-signin-con">
						<view class="my-signin-item">
							<image :src="$util.img('/upload/uniapp/signin/bg2.png')"></image>
						...
						<view class="my-signin-item">
							<image :src="$util.img('/upload/uniapp/signin/bg3.png')"></image>
							<view class="my-signin-item-num">
				...
				<view class="signin-rule">
					<view class="signin-rule-title">
						<image :src="$util.img('/upload/uniapp/signin/icon2.png')"></image>
						<text>签到规则</text>
						<image :src="$util.img('/upload/uniapp/signin/icon1.png')"></image>
					</view>
		...
		<view @touchmove.prevent.stop>
			<uni-popup ref="uniPopup" type="center" class="wap-floating" :maskClick="false">
				<view class="popup-box" @click="close()">
					...
					<image class="pic" :src="$util.img('upload/uniapp/signin/bg1.png')"></image>
...
export default {
	...
	onShow() {
		...
		if (!this.addonIsExit.membersignin) return;
		if (uni.getStorageSync('token')) {
			this.getSignInfo();
			this.getIsSign();
		} else {
			setTimeout(() => {
				this.$refs.login.open('/otherpages/member/signin/signin');
			});
		}
	},
